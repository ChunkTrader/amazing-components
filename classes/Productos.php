...
class Productos extends Coleccion{
	...
	/* Todos los metodos getItemBD deberian ser refactorizados, recibir como parametro un objeto
		...
		asÌ eliminariamos todo el jaleo de if/else. Por ejemplo getOfertas(), getNovedades(), etc.
	*/
	function getItemBD(array $opciones=null) {
		...
		if (isset($opciones['id'])) {
			...
			// Vaciamos la colecciÛn antes de cargar la tabla de la base de datos
			...
			if (isset($opciones['ofertas'])) {
				...
				// Novedades, mostramos los ˙ltimos productos aÒadidos primero. Si  'novedades'=-1
				// las mostramos por p·ginas, si hay una cantidad es que estamos mostrando las 
				// del index.
				...
				// Outlet, mostramos los productos m·s antiguos primero.
